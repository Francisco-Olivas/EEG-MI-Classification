...
class UI(QMainWindow):
    ...
    def updategraph(self):
        ...
        for i in range(0, len(self.curves)):
           ...
           ###
            if self.recording:
                self.record[i] += data[i + 1].tolist()
                if i == 0:
                    current_fase = vfases[self.index]
                    if current_fase == "Relajacion":
                        current_prompt = "relajacion"
                    else:
                        current_prompt = vpalabras[self.indexPalabra]
                    self.prompts.extend([current_prompt] * len(data[i + 1]))
                    self.state.extend([current_fase] * len(data[i + 1]))
###
    ...
    '''def update(self):
        ...
        ##
    def update(self):
        # Avanzamos al siguiente índice de fase
        self.index += 1
        
        # Si hemos llegado al final de las fases, reiniciamos
        if self.index == len(vfases):
            self.index = 0
            self.indexPalabra = random.randint(0, len(vpalabras) - 1)  # Seleccionar aleatoriamente 
            # Establecemos la palabra aleatoria en las etiquetas
            self.w.label_6.setText(vpalabras[self.indexPalabra])
            self.w.label_8.setText(vpalabras[self.indexPalabra])
            self.w.label_10.setText(vpalabras[self.indexPalabra])
        
        # Cambiamos a la siguiente fase
        self.w.stackw.setCurrentIndex(objfases[vfases[self.index]])
    
        # Actualizamos el temporizador de la fase actual
        self.timerP.setInterval(vtiempos[self.index] * 1000)'''
    ...
    # Si hemos llegado al final de las fases, reiniciamos
        if self.index == len(vfases):
            self.index = 0
            self.indexPalabra = random.randint(0, len(vpalabras) - 1)
        
        # Establecemos la imagen correspondiente a la palabra
            image_path = f'images/{vpalabras[self.indexPalabra]}.png'
        
        # Cargamos la imagen usando QPixmap
            pixmap = QPixmap(image_path)
            pixmap = QPixmap(image_path).scaled(500, 500, QtCore.Qt.KeepAspectRatio)
        
        # Asignamos la imagen a las etiquetas
            self.w.label_6.setPixmap(pixmap)
            self.w.label_8.setPixmap(pixmap)
            self.w.label_10.setPixmap(pixmap)
...
parser.add_argument('--serial-port', type=str,
                    help='serial port', required=False, default='COM9')
parser.add_argument('--board-id', type=int, help='board id, check docs to get a list of supported boards',
                    required=False, default=BoardIds.CYTON_DAISY_BOARD)#SYNTHETIC_BOARD)
